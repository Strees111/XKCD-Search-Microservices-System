document.addEventListener('DOMContentLoaded', () => {
    ...
    let statsInterval = null; // Таймер для фонового опроса
    let isFetchingStats = false; // Защита от наложения сетевых запросов
    ...
    // Безопасный запуск опроса
    function startStatsPolling() {
        stopStatsPolling(); // Сначала очищаем старый таймер перед созданием нового

        if (token) {
            fetchDbStatsAndStatus();
            if (liveIndicator) liveIndicator.classList.remove('hidden');

            // Запускаем периодический опрос каждые 3 секунды
    ...
    function switchTab(activeTabKey) {
        ...
        if (activeTabKey === 'admin') {
            updateAdminUI();
            startStatsPolling(); // Начинаем опрос при входе на вкладку админа
        } else {
            stopStatsPolling(); // Приостанавливаем опрос на других вкладках
        }
    ...
    tabs.search.btn.addEventListener('click', () => switchTab('search'));
    tabs.admin.btn.addEventListener('click', () => switchTab('admin'));
    tabs.status.btn.addEventListener('click', () => switchTab('status'));
    ...
    function updateAdminUI() {
        if (token) {
            ...
            // Чтобы избежать конфликта стилей Tailwind (hidden vs grid), переключаем их синхронно
        ...
        try {
            ...
            if (tokenText) {
                try {
                    ...
                    // Извлекаем токен и сразу очищаем его от возможных лишних кавычек
                ...
                updateAdminUI();
                startStatsPolling(); // Запускаем опрос после успешного входа
            } else {
    ...
    function logout() {
        ...
        updateAdminUI();
        stopStatsPolling(); // Останавливаем опрос при выходе
    }
    ...
    searchForm.addEventListener('submit', async (e) => {
        ...
        searchResultsPlaceholder.textContent = 'Выполнение поиска...';
        searchResults.classList.add('hidden');
        searchResultsError.classList.add('hidden');

        const isIndexed = searchType.value === 'indexed';
        const limitValue = searchLimit.value;
            ...
            <div class="space-y-4 max-h-[70vh] overflow-y-auto pr-2 animate-fade-in">
                ...
                <div class="relative group max-h-[45vh] overflow-hidden rounded-md border border-gray-200 shadow-sm bg-white flex justify-center items-center">
                    ...
                    <div class="absolute bottom-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex gap-1.5">
                        ...
                        <a href="${escapeHtml(detailedInfo.url)}" target="_blank" class="bg-indigo-600 hover:bg-indigo-700 text-white text-[10px] py-1 px-2.5 rounded shadow-sm font-semibold transition-all">Открыть оригинал</a>
                    </div>
                </div>
                ` : ''}
                <div>
    ...
    async function fetchDbStatsAndStatus() {
        ...
        try {
            ...
            if (statusData) {
                ...
                if (statusText) {
                    statusText.textContent = status === 'idle' ? 'В Ожидании (Idle)' : 'Выполняется...';
                }
            ...
            if (statsData) {
                ...
                const comicsFetched = statsData.comics_fetched !== undefined ? statsData.comics_fetched : 0;
                const comicsTotal = statsData.comics_total !== undefined ? statsData.comics_total : 0;
    ...
    document.getElementById('btn-refresh-stats').addEventListener('click', fetchDbStatsAndStatus);

    document.getElementById('btn-toggle-raw-json').addEventListener('click', (e) => {
    ...
    document.getElementById('btn-db-update').addEventListener('click', async () => {
        showActionStatus('Выполнение запроса на обновление...', 'info');
        stopStatsPolling(); // Приостанавливаем опрос на время отправки запроса
        try {
            if (!token) {
            ...
            showActionStatus(`Запрос на обновление БД успешно отправлен.${responseText ? ` ${responseText}` : ''}`.trim(), 'success');
            setTimeout(() => {
                startStatsPolling();
            }, 1500);
        } catch (err) {
            showActionStatus(`Не удалось обновить БД: ${err.message}`, 'error');
            startStatsPolling();
        }
    });

    document.getElementById('btn-db-drop').addEventListener('click', async () => {
        ...
        stopStatsPolling(); // Полностью отключаем фоновый опрос на время удаления таблиц
        try {
            if (!token) {
                showActionStatus('Ошибка: требуется авторизация. Войдите заново.', 'error');
                return;
